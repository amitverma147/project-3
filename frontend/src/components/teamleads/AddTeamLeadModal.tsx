...
export default function AddTeamLeadModal({
  ...
  useEffect(() => {
    if (isOpen) {
      if (editUser) {
        ...
        // Pre-select current user as manager if they're a manager
        setForm({
          ...EMPTY,
          managerId: isManagerRole ? (currentUser?._id ?? "") : "",
        });
        setAvailableStates([]);
        setAvailableCities([]);
      }
  ...
  }, [isOpen, editUser, currentUser, isManagerRole]);
        ...
        >
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              {inp("firstName", form.firstName, "Bob", "text", isEdit)}
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-700 mb-1">
                Last Name
              </label>
              {inp("lastName", form.lastName, "Jones", "text", isEdit)}
            </div>
          </div>
          <div>
            <label className="block text-xs font-medium text-gray-700 mb-1">
            ...
            {inp("username", form.username, "bob_jones")}
          </div>
          <div>
            <label className="block text-xs font-medium text-gray-700 mb-1">
              Email
            </label>
            {inp("email", form.email, "bob@example.com", "email")}
          </div>
          {!isEdit && (
            <div>
              <label className="block text-xs font-medium text-gray-700 mb-1">
          ...
          <div>
            ...
            <select
              ...
              disabled={loadingManagers || isManagerRole}
              className={`w-full px-3 py-2 border rounded-md shadow-sm text-sm text-black focus:outline-none focus:ring-1 focus:ring-black focus:border-black bg-white ${errors.managerId ? "border-red-500" : "border-gray-300"} ${
                loadingManagers || isManagerRole
            ...
            {isManagerRole && (
              <p className="mt-1 text-xs text-gray-500">
                Pre-selected (you are the manager)
              </p>
            )}
            {!isManagerRole && errors.managerId && (
              <p className="mt-1 text-xs text-red-600">{errors.managerId}</p>
            )}
          </div>
