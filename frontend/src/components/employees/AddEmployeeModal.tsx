...
export default function AddEmployeeModal({
  ...
  useEffect(() => {
    if (isOpen) {
      if (editUser) {
        ...
        // Pre-select current user as team lead if they're a team lead
        setForm({
          ...EMPTY,
          teamLeadId: isTeamLeadRole ? (currentUser?._id ?? "") : "",
        });
        setAvailableStates([]);
        setAvailableCities([]);
      }
  ...
  }, [isOpen, editUser, currentUser, isTeamLeadRole, fetchTeamLeads]);
  ...
  const validateDynamicField = (
    ...
    switch (field) {
      case "firstName":
        return !nextForm.firstName.trim() ? "Required" : undefined;
      case "lastName":
        return !nextForm.lastName.trim() ? "Required" : undefined;
      case "email":
      ...
      case "teamLeadId":
        return !isTeamLeadRole && !nextForm.teamLeadId
          ? "Select a team lead"
          : undefined;
  ...
  const validate = (): boolean => {
    ...
    // Only validate teamLeadId if user is not a team lead (since it's auto-assigned for team leads)
    if (!isTeamLeadRole && !form.teamLeadId)
      e.teamLeadId = "Select a team lead";
        ...
        >
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              {inp("firstName", form.firstName, "Jane", "text", isEdit)}
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-700 mb-1">
                Last Name
          ...
          <div>
            ...
            {inp("username", form.username, "jane_smith")}
          </div>
          <div>
            <label className="block text-xs font-medium text-gray-700 mb-1">
              Email
            </label>
            {inp("email", form.email, "jane@example.com", "email")}
          </div>
          {!isEdit && (
            <div>
              <label className="block text-xs font-medium text-gray-700 mb-1">
          ...
          {!isTeamLeadRole ? (
            <div>
              ...
              <select
                ...
                onChange={(e) => setField("teamLeadId", e.target.value)}
                disabled={loadingTLs}
                className={`w-full px-3 py-2 border rounded-md shadow-sm text-sm text-black focus:outline-none focus:ring-1 focus:ring-black focus:border-black bg-white ${errors.teamLeadId ? "border-red-500" : "border-gray-300"} ${loadingTLs ? "opacity-50 cursor-not-allowed" : ""}`}
              >
                <option value="">
                  {loadingTLs ? "Loading…" : "Select a team lead"}
                </option>
                {teamLeads.map((t) => (
                  <option key={t._id} value={t._id}>
                    {t.firstName || t.fname} {t.lastName || t.lname}
                  </option>
                ))}
              </select>
              {errors.teamLeadId && (
                <p className="mt-1 text-xs text-red-600">{errors.teamLeadId}</p>
              )}
            </div>
          ) : (
            <div className="bg-blue-50 border border-blue-200 rounded-md p-3">
              <p className="text-sm text-blue-800 font-medium">
                Adding employee under your supervision
              </p>
              <p className="text-xs text-blue-600 mt-1">
                You are automatically assigned as the team lead for this
                employee.
              </p>
            </div>
          )}
