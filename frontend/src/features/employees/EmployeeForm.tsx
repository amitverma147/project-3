...
interface EmployeeFormProps {
  ...
  teamLeadOptions?: TeamLeadOption[];
...
export default function EmployeeForm({
  ...
  const schema = useMemo(
    () => buildEmployeeSchema({ isEdit, isTeamLeadRole }),
    [isEdit, isTeamLeadRole],
  );
  ...
  const onValidSubmit = async (values: EmployeeFormValues) => {
    setSubmitting(true);
    try {
    ...
    >
      ...
      {!isTeamLeadRole ? (
        <FormField
          label="Team Lead"
          type="select"
          placeholder={loadingTeamLeads ? "Loading..." : "Select Team Lead"}
          options={teamLeadOptions}
          register={register("teamLeadId")}
          error={errors.teamLeadId}
          disabled={disabled || loadingTeamLeads}
        />
      ) : (
        <div className="rounded-md border border-blue-200 bg-blue-50 p-3">
          <p className="text-sm font-medium text-blue-800">
            You are automatically assigned as team lead.
          </p>
        </div>
      )}
